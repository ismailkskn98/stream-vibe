...
import type { CastType, FetchResultType, MovieDetailType, MoviesType, ReviewsType, watchProvidersType } from "@/types";
import React from "react";

export default async function page({ params }: { params: Promise<{ id: string; slug: string }> }) {
  const id = (await params).id;
  const [trendMoviesPage1, trendMoviesPage2, movie, cast, reviews, watchProviders] = await Promise.all([
    ...
    movieDetailFetch<watchProvidersType>(id, "watch/providers", "language=us-US"),
  ]);
